/**
 * Agent Management Service (Database Version)
 * Create, configure, and manage custom agents using MySQL database
 */
...
export class AgentManager {
  constructor() {
    // No longer need file paths - using database
  }

  /**
   * Get all default agents (built-in) from database
   */
  async getDefaultAgents(): Promise<AgentConfig[]> {
    try {
      ...
      return agents.map(agent => ({
        id: agent.id.replace('builtin-', ''), // Remove prefix for backward compatibility
        name: agent.name,
        role: agent.role,
        emoji: agent.emoji,
        color: agent.color,
        ...
        configVariables: agent.configVariables as AgentConfigVariable[] | undefined,
        isCustom: false,
        createdAt: agent.createdAt.toISOString(),
        updatedAt: agent.updatedAt.toISOString(),
      }));
    } catch (error) {
      console.error('Error loading built-in agents:', error);
  ...
  /**
   * Get hardcoded default agents (fallback only)
   */
  private getHardcodedDefaultAgents(): AgentConfig[] {
    return [
      {
        id: '1',
        name: 'Peter van Dijk',
...
Always notify stakeholders about deployment status.`,
  ...
  async getAllAgents(): Promise<AgentConfig[]> {
    const defaultAgents = await this.getDefaultAgents();
  ...
  async createAgent(agent: Omit<AgentConfig, 'id' | 'isCustom' | 'createdAt' | 'updatedAt'>): Promise<AgentConfig> {
    ...
    return {
      ...
      capabilities: created.capabilities as string[],
      tools: created.tools as string[],
      model: created.model as 'sonnet' | 'opus' | 'haiku',
      category: created.category as AgentCategory,
      configVariables: created.configVariables as AgentConfigVariable[] | undefined,
      isCustom: true,
  ...
  async updateAgent(id: string, updates: Partial<AgentConfig>): Promise<AgentConfig> {
    ...
    const updated = await prisma.customAgent.update({
      ...
      data: {
        ...
        ...(updates.configVariables !== undefined && { configVariables: updates.configVariables || null }),
      },
    });
    ...
    return {
      ...
      model: updated.model as 'sonnet' | 'opus' | 'haiku',
      category: updated.category as AgentCategory,
      configVariables: updated.configVariables as AgentConfigVariable[] | undefined,
      isCustom: true,
...
`;
  ...
  /**
   * Save custom agents to database (no longer needed - kept for backward compatibility)
   */
  private async saveCustomAgents(agents: AgentConfig[]): Promise<void> {
    // This method is deprecated - agents are now saved directly via createAgent/updateAgent
    console.warn('saveCustomAgents is deprecated');
  }
